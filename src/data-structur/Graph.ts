...
export default class Graph<T, K extends {}> {
  vertices: { [key: string]: GraphVertex<T> };
  edges: { [key: string]: GraphEdge<T, K> };
  isDirected: boolean;
  /**
  ...
  getWeight() {
    return this.getAllEdges().reduce((weight, graphEdge) => {
      return weight + Number(graphEdge.weight);
    }, 0);
  }
    ...
    vertices.forEach((vertex, vertexIndex) => {
      vertex.getNeighbors().forEach((neighbor) => {
        ...
        adjacencyMatrix[vertexIndex][neighborIndex] =
          this.findEdge(vertex, neighbor)?.weight || Infinity;
      });
    });
